...
svi_sub <- svi_sub %>%
  dplyr::select(FIPS, starts_with(c("SPL_THEME")))
...
global_dat <- dat_full %>%
  ...
  select(GEOID10, avgReleasesLb_10yr, LWB_ADJRATE, starts_with("SPL_"))
global_dat <- 
...
vars <- c(
  "avgReleasesLb_10yr", "SPL_THEME1", "SPL_THEME2", "SPL_THEME3", "SPL_THEME4"
)
...
```{r, message=FALSE}
# Fit global model
lm_full <- lm(LWB_ADJRATE ~ avgReleasesLb_10yr + SPL_THEME1 + SPL_THEME2 + SPL_THEME3 + SPL_THEME4
              , data = global_dat)
...
Generally, group means >0 suggest a positive relationship between the explanatory variable and outcome and vice versa. Sometimes $\beta$ coefficients are both positive and negative – this can make interpretation difficult, requiring deeper interrogation of how local relationships play out. In the plots above, we see that `avgReleasesLb_10yr`, `SPL_THEME2`, `SPL_THEM3` and `SPL_TMEME4` Next, we will map these coefficients to see where this is occurring.
...
p3_gwr <- plot.gwr.coefs(gwr_results_dat$SDF, "SPL_THEME1", gwr_results_dat$SDF$SPL_THEME1_TV)
p4_gwr <- plot.gwr.coefs(gwr_results_dat$SDF, "SPL_THEME2", gwr_results_dat$SDF$SPL_THEME2_TV)
p5_gwr <- plot.gwr.coefs(gwr_results_dat$SDF, "SPL_THEME3", gwr_results_dat$SDF$SPL_THEME3_TV)
p6_gwr <- plot.gwr.coefs(gwr_results_dat$SDF, "SPL_THEME4", gwr_results_dat$SDF$SPL_THEME4_TV)
...
1. Formatted data
2. Ran spatial statistical tests
3. Analyzed model diagnostics, and 
4. Plotted a simple map of model results. 
...
table_data <- data.frame(
  ...
  LM = c(summary(lm_full)$r.squared %>% signif(3)
       ...
       , AIC(lm_full) %>% signif(3)
       , BIC(lm_full) %>% signif(3)
  ...
  GWR = c(gwr_results_dat$GW.diagnostic$gw.R2 %>% signif(3)
       ...
       , gwr_results_dat$GW.diagnostic$AIC %>% signif(3)
       , gwr_results_dat$GW.diagnostic$BIC %>% signif(3)
...
In the plots above, we see that `SPL_THEME1` and `SPL_TMEME3` (to an extent) are both positive and negative. Again, we will map these coefficients to see where this is occurring. 

$\textbf{A note on MGWR iterations:}$ In the HTML document you will see that we ran the MGWR with a maximum of 100 iterations in hopes that the model will converge using the CVR backfitting criterion. Even though it doesn't converge with this adjustment, we do see different results for multiple model diagnostics, holding everything else constant. Most notably, $\beta$ coefficients for `SPL_THEME1` are now solidly positive. While you can make meaningful inference about model strength with one or a few iterations, it's good practice to let the model approach convergence.
...
p3_mgwr <- plot.gwr.coefs(mgwr_results_dat_full$SDF, "SPL_THEME1", mgwr_results_dat_full$SDF$SPL_THEME1_TV)
p4_mgwr <- plot.gwr.coefs(mgwr_results_dat_full$SDF, "SPL_THEME2", mgwr_results_dat_full$SDF$SPL_THEME2_TV)
p5_mgwr <- plot.gwr.coefs(mgwr_results_dat_full$SDF, "SPL_THEME3", mgwr_results_dat_full$SDF$SPL_THEME3_TV)
p6_mgwr <- plot.gwr.coefs(mgwr_results_dat_full$SDF, "SPL_THEME4", mgwr_results_dat_full$SDF$SPL_THEME4_TV)
...
table_data <- data.frame(
  ...
  MGWR = c(mgwr_results_dat_full$GW.diagnostic$R2.val %>% signif(3)
       ...
       , mgwr_results_dat_full$GW.diagnostic$AIC %>% signif(3)
       , mgwr_results_dat_full$GW.diagnostic$BIC %>% signif(3)
